...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES['file']['name'])) {
        ...
        if (!in_array(strtolower($fileExtension), $allowedExtensions)) {
            ...
            $new_name_for_file = $random_new_name . '.' . $fileExtension;
            move_uploaded_file($_FILES['file']['tmp_name'], "../profile_img/" . $new_name_for_file);
            $img = "profile_img/" . $new_name_for_file;
        }
    }
    ...
    if (!empty($_SESSION['error'])) {
        ...
        $query = "UPDATE register_table SET register_full_name='$fullname', register_user_name='$username', register_password='$hashed_password', register_contact_number='$contact', email='$email', register_profile_picture='$img' WHERE register_id=$id";
